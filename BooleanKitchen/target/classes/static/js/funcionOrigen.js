function eliminarOrigen(button){
    ...
    var TablaOrigen = document.getElementById("Tabla"); 

    xmlhttp.open("GET", "/c_origen/DeleteOrigen/" + id+"/"+url);
    xmlhttp.send();
...
function agregarPrefijo(inputField) {
    if (!inputField.value.startsWith('ORG-')) {
        inputField.value = 'ORG-';
    }
...
function cambiarForm(accion) {
    document.getElementById('Formw').action = accion;
}